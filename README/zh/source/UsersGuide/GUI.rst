...
.. Last-Updated: 五 10月 25 22:03:20 2024 (+0800)
..           By: Hongyi Wu(吴鸿毅)
..     Update #: 9
...
选择、或者不选择 **Online Mode** 选项之后。如果是机箱上电之后的第一次初始化，则按 **Boot** 按钮开启初始化过程，这个过程将会加载固件、参数等信息。除此之外都可以通过 **Relink** 按钮来继续控制获取系统。可看到最下方 *Information* 栏目中的状态变化。

系统初始化或者连接成功后，再次确认 *Setup* 栏中的获取文件存放路径、文件名、文件编号是否有问题，如果有问题则直接修改，确认之后按 **Complete**。
