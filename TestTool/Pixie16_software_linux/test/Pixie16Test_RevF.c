...
int main(int argc, char *argv[])
{
  if(argc != 2)
    {
      std::cout<< "run:   ./xxx [runnumber]" <<std::endl;
      exit(1);
    }
  int N = atoi(argv[1]);
  ...
  retval = Pixie16BootModule (ComFPGAConfigFile, SPFPGAConfigFile, TrigFPGAConfigFile,
			      DSPCodeFile, DSPParFile, DSPVarFile, NumModules, 0x7F); // 0x7F
  if (retval < 0)
    {
  ...
  while(1)
	...
	{
	  EndOfRunRead = 0;
	  sprintf(filnam, "data/%04d_mod%d.bin",N, k);
  ...
  for(int i=0;i<NumModules;i++)
    {	
      sprintf(filnam, "data/histogram_%04d_mod%d.bin",N, i);
