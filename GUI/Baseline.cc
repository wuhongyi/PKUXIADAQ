  ...
  for (int i = 0; i < 16; i++)
    {
      ...
      if(retval < 0) ErrorInfo("Baseline.cc", "load_info(...)", "Pixie16ReadSglChanPar/BLCUT", retval);  
      ...
      if(retval < 0) ErrorInfo("Baseline.cc", "load_info(...)", "Pixie16ReadSglChanPar/BASELINE_PERCENT", retval);  
      sprintf(text, "%d", (int)ChanParData);
  ...
  for (int i = 0; i < 16; i++)
    {
      ...
      if(retval < 0) ErrorInfo("Baseline.cc", "change_values(...)", "Pixie16WriteSglChanPar/BLCUT", retval);  
      percent = NumEntry[2][i]->GetNumber();
      retval = Pixie16WriteSglChanPar((char*)"BASELINE_PERCENT", percent, mod, i);
      if(retval < 0) ErrorInfo("Baseline.cc", "change_values(...)", "Pixie16WriteSglChanPar/BASELINE_PERCENT", retval);  
    }
