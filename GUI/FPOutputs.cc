...
// Last-Updated: 日 11月  6 20:07:49 2016 (+0800)
//           By: Hongyi Wu(吴鸿毅)
//     Update #: 33
...
int FPOutputs::load_info(Long_t mod)
{
  ...
  if(retval < 0) ErrorInfo("FPOutputs.cc", "load_info(...)", "Pixie16ReadSglChanPar/TrigConfig0", retval);
...
int FPOutputs::change_values(Long_t mod)
{
  ...
  int retval;
  retval = Pixie16ReadSglModPar((char*)"TrigConfig0", &ModParData, mod);
  if(retval < 0) ErrorInfo("FPOutputs.cc", "change_values(...)", "Pixie16ReadSglModPar/TrigConfig0", retval);
  ...
  if(retval < 0) ErrorInfo("FPOutputs.cc", "change_values(...)", "Pixie16WriteSglModPar/TrigConfig0", retval);
