  ...
  for (int i = 0; i < 16; i++)
    {
      ...
      if(retval < 0) ErrorInfo("Tau.cc", "load_info(...)", "Pixie16ReadSglChanPar/TAU", retval);
  ...
  for (int i = 0; i < 16; i++)
    {
      ...
      if(retval < 0) ErrorInfo("Tau.cc", "change_values(...)", "Pixie16WriteSglChanPar/TAU", retval);
    }
...
void Tau::findtau(short int mod)
{
  ...
  if(retval<0){
    if(retval < 0) ErrorInfo("Tau.cc", "findtau(...)", "Pixie16TauFinder", retval);
    switch (retval) {
    case -1:
