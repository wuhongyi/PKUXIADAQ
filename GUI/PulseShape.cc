  ...
  for (int i = 0; i < 16; i++)
    {
      ...
      if(retval < 0) ErrorInfo("PulseShape.cc", "change_values(...)", "Pixie16WriteSglChanPar/TRACE_LENGTH", retval);
      delay = NumEntry[2][i]->GetNumber();
      retval = Pixie16WriteSglChanPar((char*)"TRACE_DELAY", delay, mod, i);
      if(retval < 0) ErrorInfo("PulseShape.cc", "change_values(...)", "Pixie16WriteSglChanPar/TRACE_DELAY", retval);
    }
