...
int main(void)
{
  ...
  std::string s4 = webdata.substr(n4+4, n5-n4-4);
  std::string s5 = webdata.substr(n5+4);

  // printf("%s  %s  %s  %s %s\n",s0.c_str(),s1.c_str(), s2.c_str(), s3.c_str(), s4.c_str());
  ...
  // select ts from statistics where crate = 0 and date_sub(now(),interval 1 hour) <= ts;
  if(data4 == 0)
    sprintf(chartmp,"select * from statistics where crate = %d and date_sub(now(),interval 1 hour) <= ts",data0);
  else if(data4 == 1)
    sprintf(chartmp,"select * from statistics where crate = %d and date_sub(now(),interval 3 hour) <= ts",data0);
  else if(data4 == 2)
    sprintf(chartmp,"select * from statistics where crate = %d and date_sub(now(),interval 6 hour) <= ts",data0);
  else if(data4 == 3)
    sprintf(chartmp,"select * from statistics where crate = %d and date_sub(now(),interval 12 hour) <= ts",data0);
  else if(data4 == 4)
    sprintf(chartmp,"select * from statistics where crate = %d and date_sub(now(),interval 1 day) <= ts",data0);
  else if(data4 == 5)
    sprintf(chartmp,"select * from statistics where crate = %d and date_sub(now(),interval 3 day) <= ts",data0);
  else if(data4 == 6)
    sprintf(chartmp,"select * from statistics where crate = %d and date_sub(now(),interval 7 day) <= ts",data0);
  else
    sprintf(chartmp,"select * from statistics where crate = %d and date_sub(now(),interval 1 hour) <= ts",data0);
	...
	if(run == run_new)
	  {
	    ...
	    if(tempinputcountrate < 0) tempinputcountrate = -10;
	    if(data5 > 0 && data5 < tempinputcountrate) tempinputcountrate = data5;
	    tempoutputcountrate = int((ChanEvents_new[j]-ChanEvents[j])/(RealTime_new-RealTime) +0.5);
	    if(tempoutputcountrate < 0) tempoutputcountrate = -10;
	     if(data5 > 0 && data5 < tempoutputcountrate) tempoutputcountrate = data5;
	    if(data3 == 0)
