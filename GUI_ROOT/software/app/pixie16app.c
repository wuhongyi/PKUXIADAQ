...
PIXIE16APP_EXPORT int PIXIE16APP_API Pixie16WriteSglChanPar(const char* ChanParName,
    ...
    unsigned int chantrigstretch, cfdthresh, integrator, resetdelay;
    ...
    if (strcmp(ChanParName, "TRIGGER_RISETIME") == 0) {
	  ...
	  resetdelay = (unsigned int)ROUND(ChanParData * (double)(Module_Information[ModNum].Module_ADCMSPS / 5));
        ...
        // Range check for ResetDelay
        if(resetdelay > RESETDELAY_MAX)
	  {
            resetdelay = RESETDELAY_MAX;
	  }
        ...
        if(retval < 0)
	  {
	    Pixie_Print_Error(
			      PIXIE_FUNC,
			      "ProgramFippi failed in module %d channel %d after downloading ResetDelay, retval=%d",
			      ModNum, ChanNum, retval);
            return(-4);
	  }
      }
	...
	double         *cfd ) 
{
  ...
  if(RcdTrace == NULL)
    {
      Pixie_Print_Error(PIXIE_FUNC, "(HongyiWuPixie16ComputeCFDFiltersOffline): Null pointer *RcdTrace");
      return(-1);
    }
  ...
  if(cfd == NULL)
    {
      Pixie_Print_Error(PIXIE_FUNC, "(HongyiWuPixie16ComputeCFDFiltersOffline): Null pointer *cfd");
      return(-2);
    }
  ...
  if(RcdTraceLength < D+B+L)
    {
      Pixie_Print_Error(PIXIE_FUNC, "(HongyiWuPixie16ComputeCFDFiltersOffline): the length of recorded trace is too short");
      return(-3);
    }
	...
	double         *cfds )             // cfd response, the same scale as the fast filter
{
	...
	if(RcdTrace == NULL)
	{
		Pixie_Print_Error(PIXIE_FUNC, "(HongyiWuPixie16ComputeFastFiltersOffline): Null pointer *RcdTrace");
		return(-1);
	}
	...
	if(fastfilter == NULL)
	{
		Pixie_Print_Error(PIXIE_FUNC, "(HongyiWuPixie16ComputeFastFiltersOffline): Null pointer *fastfilter");
		return(-2);
	}
	...
	if(cfd == NULL)
	{
		Pixie_Print_Error(PIXIE_FUNC, "(HongyiWuPixie16ComputeFastFiltersOffline): Null pointer *cfd");
		return(-3);
	}

	if(ModuleNumber >= PRESET_MAX_MODULES)
	{
		Pixie_Print_Error(PIXIE_FUNC, "(HongyiWuPixie16ComputeFastFiltersOffline): Target module number is invalid %d", ModuleNumber);
		return(-4);
	}
	...
	if(RcdTraceLength < ((2*FastLen + FastGap)*2))
	{
		Pixie_Print_Error(PIXIE_FUNC, "(HongyiWuPixie16ComputeFastFiltersOffline): the length of recorded trace is too short");
		return(-5);
	}
	...
	{
		Pixie_Print_Error(PIXIE_FUNC, "(HongyiWuPixie16ComputeFastFiltersOffline): can't open list-mode file %s", FileName);
		return(-6);
	}
	...
	int            pointtobl )         // Average number of estimated waveform baselines
{
	...
	if(RcdTrace == NULL)
	{
		Pixie_Print_Error(PIXIE_FUNC, "(HongyiWuPixie16ComputeSlowFiltersOfflineAverageBaseline): Null pointer *RcdTrace");
		return(-1);
	}
	...
	if(slowfilter == NULL)
	{
		Pixie_Print_Error(PIXIE_FUNC, "(HongyiWuPixie16ComputeSlowFiltersOfflineAverageBaseline): Null pointer *slowfilter");
		return(-2);
	}
	
	if(ModuleNumber >= PRESET_MAX_MODULES)
	{
		Pixie_Print_Error(PIXIE_FUNC, "(HongyiWuPixie16ComputeSlowFiltersOfflineAverageBaseline): Target module number is invalid %d", ModuleNumber);
		return(-3);
	}
	...
	{
		Pixie_Print_Error(PIXIE_FUNC, "(HongyiWuPixie16ComputeSlowFiltersOfflineAverageBaseline): the length of recorded trace is too short");
		return(-4);
	}
	...
	{
		Pixie_Print_Error(PIXIE_FUNC, "(HongyiWuPixie16ComputeSlowFiltersOfflineAverageBaseline): can't open list-mode file %s", FileName);
		return(-5);
	}
	...
	int            pointtobl )         // Average number of estimated waveform baselines
{
	...
	if(RcdTrace == NULL)
	{
		Pixie_Print_Error(PIXIE_FUNC, "(HongyiWuPixie16ComputeSlowFiltersOfflineExtendBaseline): Null pointer *RcdTrace");
		return(-1);
	}
	...
	if(slowfilter == NULL)
	{
		Pixie_Print_Error(PIXIE_FUNC, "(HongyiWuPixie16ComputeSlowFiltersOfflineExtendBaseline): Null pointer *slowfilter");
		return(-2);
	}
	
	if(ModuleNumber >= PRESET_MAX_MODULES)
	{
		Pixie_Print_Error(PIXIE_FUNC, "(HongyiWuPixie16ComputeSlowFiltersOfflineExtendBaseline): Target module number is invalid %d", ModuleNumber);
		return(-3);
	}
	...
	{
		Pixie_Print_Error(PIXIE_FUNC, "(HongyiWuPixie16ComputeSlowFiltersOfflineExtendBaseline): the length of recorded trace is too short");
		return(-4);
	}
	...
	{
		 Pixie_Print_Error(PIXIE_FUNC, "(HongyiWuPixie16ComputeSlowFiltersOfflineExtendBaseline): can't open list-mode file %s", FileName);
		return(-5);
	}
